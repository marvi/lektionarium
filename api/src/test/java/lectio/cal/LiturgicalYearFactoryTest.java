...
public class LiturgicalYearFactoryTest extends TestCase {
   ...
   public void testLiturgicalYearNotEqualToLiturgicalYear() {
     ...
     assertEquals("Andra söndagen i advent", d.name()); // Replaced d.name with d.name()
   }
  ...
  public void testGetPrevDay() {
    ...
    while (it.hasNext()) {
      ...
      assertEquals(pairs.getValue(), fact.getPreviousDay(pairs.getKey()).date()); // Replaced getDate() with date()
    }
  }
  ...
  public void testGetNextDay() {
    ...
    while (it.hasNext()) {
      Map.Entry<LocalDate, LocalDate> pairs = (Map.Entry) it.next();
      assertEquals(pairs.getValue(), fact.getNextDay(pairs.getKey()).date()); // Replaced getDate() with date()
    }
  }
